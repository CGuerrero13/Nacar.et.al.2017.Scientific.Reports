...
clc;
clear all
cd /Volumes/Karlos_SAP/Databases/BrainVision/Databases_paper/Theta
...
%Scaling with respect to negative values (just for plotting)
% and it is not use for STAT analysis
y=2*min(min(y))*-1 + y;

e(1,1) = std(theta_mono_nat)/length(theta_mono_nat);
...
pause;
%%%%%%% STATS: parametric test (after normalization test)
